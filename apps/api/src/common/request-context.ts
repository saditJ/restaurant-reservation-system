...
class RequestContextCls {
  ...
  run<T>(req: Request, cb: () => T) { return this.als.run({ req }, cb); }
  get req() { return this.als.getStore()?.req; }
  get tenantId(): string | undefined { return this.req?.tenantId; }
  set tenantId(val: string | undefined) { if (this.req) this.req.tenantId = val; }
}
