...
function normalizeRoute(request: Request): string {
  ...
  const combined = `${baseUrl}${routePath}` || request.path || request.originalUrl || 'unknown';
...
export function createRequestTimingMiddleware(metricsService: MetricsService) {
  return function requestTimingMiddleware(req: Request, res: Response, next: NextFunction) {
    if (req.path === '/metrics') {
      next();
      return;
    }
